...
class AbstractIntegration(BaseModel):
    ...
    test_event_type: Optional[StrictStr] = Field(default=None, alias="testEventType")
    test_severity: Optional[StrictStr] = Field(default=None, alias="testSeverity")
    test_state: Optional[StrictStr] = Field(default=None, alias="testState")
    timezone: Optional[StrictStr] = Field(default=None, description="The timezone to be used for DateTime")
    __properties: ClassVar[List[str]] = ["id", "kind", "name", "testEventType", "testSeverity", "testState", "timezone"]

    @field_validator('test_event_type')
    def test_event_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['INFRA_SMART_ALERT', 'LOG_SMART_ALERT', 'APPLICATION_SMART_ALERT', 'BUILT_IN_EVENT', 'CUSTOM_EVENT']):
            raise ValueError("must be one of enum values ('INFRA_SMART_ALERT', 'LOG_SMART_ALERT', 'APPLICATION_SMART_ALERT', 'BUILT_IN_EVENT', 'CUSTOM_EVENT')")
        return value

    @field_validator('test_severity')
    def test_severity_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['WARNING', 'CRITICAL']):
            raise ValueError("must be one of enum values ('WARNING', 'CRITICAL')")
        return value

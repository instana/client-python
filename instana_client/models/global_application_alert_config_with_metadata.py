...
class GlobalApplicationAlertConfigWithMetadata(BaseModel):
    ...
    display_url: Optional[StrictStr] = Field(default=None, description="Absolute URL of this configuration in the Instana UI.", alias="displayUrl")
    ...
    group_by: Optional[Annotated[List[GroupByTag], Field(min_length=0, max_length=1)]] = Field(default=None, description="The grouping tags used to group the metric results. Only applicable when evaluationType is CUSTOM.", alias="groupBy")
    ...
    __properties: ClassVar[List[str]] = ["alertChannelIds", "alertChannels", "applicationIds", "applications", "boundaryScope", "builtIn", "created", "customPayloadFields", "description", "displayUrl", "enabled", "evaluationType", "gracePeriod", "granularity", "groupBy", "id", "includeInternal", "includeSynthetic", "initialCreated", "name", "readOnly", "rule", "rules", "severity", "tagFilterExpression", "threshold", "timeThreshold", "triggering"]
    ...
    def evaluation_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PER_AP', 'PER_AP_SERVICE', 'PER_AP_ENDPOINT', 'CUSTOM']):
            raise ValueError("must be one of enum values ('PER_AP', 'PER_AP_SERVICE', 'PER_AP_ENDPOINT', 'CUSTOM')")
        return value
